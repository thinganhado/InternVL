...
DEFAULT_P1_JSON_ROOT = "/datasets/work/dss-deepfake-audio/work/data/datasets/interspeech/baseline_strongVLM/InternVL3-78B_test"
DEFAULT_MODEL_ID = "/datasets/work/dss-deepfake-audio/work/data/datasets/interspeech/VLM/InternVL3-78B/"
DEFAULT_OUTPUT_DIR = "/scratch3/che489/Ha/interspeech/VLM/InternVL/query2_outputs_from_p1json"
...
def _img_stem_candidates(stem: str):
    ...
    if not s:
        return []
    variants = [s, s.replace("_LA_D_", "_"), s.replace("_LA_D_", "")]
    if s.endswith("_img_edge_number_axes"):
        base = s[: -len("_img_edge_number_axes")]
        variants.extend([base, base.replace("_LA_D_", "_"), base.replace("_LA_D_", "")])
    seen, out = set(), []
    for v in variants:
...
def _load_prompt1_output(sample_id_candidates, root: Path, fallback_by_stem=None, img_stem: str = ""):
    ...
    for sample_id in sample_id_candidates:
        ...
        if not p.exists():
            ...
            continue
        payload = json.loads(p.read_text(encoding="utf-8-sig"))
        return sample_id, _extract_numbers_list_text(payload.get("response", ""))
    if fallback_by_stem:
...
def _discover_items(args: argparse.Namespace):
    ...
    with meta_csv.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row_idx, row in enumerate(reader, start=1):
            ...
            unique_sample_id = sample_id if sample_id not in used_sample_ids else f"{sample_id}__row{row_idx}"
            ...
            try:
                ...
                if len(missing_examples) < 5:
                    missing_examples.append({"row": row_idx, "img_stem": img_path.stem, "candidates": sample_id_candidates})
...
def _generate_one(model, tokenizer, pixel_values, question, max_new_tokens, do_sample, temperature, top_p):
    generation_config = {"max_new_tokens": max_new_tokens, "do_sample": bool(do_sample and temperature > 0.0)}
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run InternVL query2 prompt with prompt1_output loaded from baseline JSON.")
...
def main():
    ...
    with output_jsonl.open(mode, encoding="utf-8", buffering=1) as jsonl_fp:
        for idx, item in enumerate(items, start=1):
            ...
            sample_dir = output_dir / item["crop_method"].lower() / _safe_path_component(item["sample_id"])
